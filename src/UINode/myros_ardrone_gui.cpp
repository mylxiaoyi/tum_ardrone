...
myros_ardrone_gui::myros_ardrone_gui(QWidget *parent)
    : QWidget(parent)
{
	ui.setupUi(this);
	rosThread = NULL;
    ...
    getdirtxt(	ros::package::getPath("myros_ardrone") + std::string("/flightPlans/"),files);
...
myros_ardrone_gui::~myros_ardrone_gui()
{
}

// clicked functions
void myros_ardrone_gui::LandClicked()
{
	rosThread->sendLand();
}
void myros_ardrone_gui::TakeoffClicked()
{
	rosThread->sendTakeoff();
}
void myros_ardrone_gui::ToggleCamClicked()
{
...
void myros_ardrone_gui::EmergencyClicked()
{
	rosThread->sendToggleState();
}

void myros_ardrone_gui::ClearClicked()
{
	rosThread->publishCommand("c clearCommands");
}
void myros_ardrone_gui::SendClicked()
{
...
void myros_ardrone_gui::ClearSendClicked()
{
	ClearClicked();
	SendClicked();
}
void myros_ardrone_gui::ResetClicked()
{
...
void myros_ardrone_gui::LoadFileChanged(QString val)
{
	if(val == "")
		ui.plainTextEditSendCommand->setPlainText("");
	else
	{
        std::string path = ros::package::getPath("myros_ardrone") + std::string("/flightPlans/") + val.toStdString();
...
void myros_ardrone_gui::ToggledUseHovering(int val)
{
	useHovering = (val != 0);
}

void myros_ardrone_gui::ToggledPingDrone(int val)
{
	pingThread->measure = (val != 0);
}

// change control source functions
void myros_ardrone_gui::ControlSourceChanged()
{
...
void myros_ardrone_gui::setControlSourceSlot(int cont)
{
...
void myros_ardrone_gui::setCountsSlot(unsigned int nav,unsigned int control,unsigned int pose,unsigned int joy)
{
	char buf[100];
...
void myros_ardrone_gui::setPingsSlot(int p500, int p20000)
{
	char buf[100];
...
void myros_ardrone_gui::addLogLineSlot(QString s)
{
	ui.plainTextEditMessages->appendPlainText(s);
}
void myros_ardrone_gui::setAutopilotInfoSlot(QString s)
{
	ui.plainTextEditAutopilotStatus->setPlainText(s);
}
void myros_ardrone_gui::setStateestimationInfoSlot(QString s)
{
	ui.plainTextEditStateestimationStatus->setPlainText(s);
}
void myros_ardrone_gui::setMotorSpeedsSlot(QString s)
{
	ui.labelDroneMotors->setText(s);
}
void myros_ardrone_gui::closeWindowSlot()
{
	closeWindow();
}
...
void myros_ardrone_gui::setCounts(unsigned int nav,unsigned int control,unsigned int pose,unsigned int joy)
{
	emit setCountsSignal(nav, control, pose, joy);
}
void myros_ardrone_gui::setControlSource(ControlSource cont)
{
	emit setControlSourceSignal((int)cont);
}
void myros_ardrone_gui::addLogLine(std::string s)
{
    std::cout << s << std::endl;
	emit addLogLineSignal(QString(s.c_str()));
}
void myros_ardrone_gui::setAutopilotInfo(std::string s)
{
	emit setAutopilotInfoSignal(QString(s.c_str()));
}
void myros_ardrone_gui::setMotorSpeeds(std::string s)
{
	emit setMotorSpeedsSignal(QString(s.c_str()));
}
void myros_ardrone_gui::setStateestimationInfo(std::string s)
{
	emit setStateestimationInfoSignal(QString(s.c_str()));
}
void myros_ardrone_gui::setPings(int p500, int p20000)
{
	emit setPingsSignal(p500, p20000);
}
void myros_ardrone_gui::closeWindow()
{
...
// KB control stuff
int myros_ardrone_gui::mapKey(int k)
{
	switch(k)
	{
		case 74: //j
...
void myros_ardrone_gui::keyReleaseEvent( QKeyEvent * key)
{
...
void myros_ardrone_gui::keyPressEvent( QKeyEvent * key)
{
...
ControlCommand myros_ardrone_gui::calcKBControl()
{
