...
GLuint Model3DS::_GenerateDisplayList( Lib3dsFile * pModel, bool bWireframe )
{
  ...
  if ( list !=0 )
  {
    ...
    Lib3dsMesh *head = pModel->meshes;
    while (head != NULL)
    {
      //Lib3dsMesh * mesh = pModel->meshes[mm];
      Lib3dsMesh * mesh = head;
      //Vector3D * normals = new Vector3D[ 3* mesh->nfaces];
      Vector3D * normals = new Vector3D[ 3* mesh->faces];
      ...
      for( int ff = 0; ff < mesh->faces; ff++ )
      {
        ...
        for( int vv = 0; vv < 3; vv++ )
        {
          ...
          glVertex3fv( mesh->pointL[ index ].pos );
        }
      }
...
void Model3DS::_LoadTextures( Lib3dsFile * pModel )
{
  ...
  Lib3dsMaterial *head = pModel->materials;
  while (head != NULL)
  {
    string sTexFile = head->texture1_map.name;
    if( !sTexFile.empty() ) {
