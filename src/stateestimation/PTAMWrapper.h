...
public:
    ...
    // for detect vanish point
    int detect_vanish_point(cv::Mat &image, cv::Point &vp, cv::Point &p_left, cv::Point &p_right);
    int k_means(double** data,int K,long int N,int D, double **now_center,int *label);
