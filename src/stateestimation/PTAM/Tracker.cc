...
Tracker::Tracker(ImageRef irVideoSize, const ATANCamera &c, Map &m,
                 MapMaker &mm)
    : mMap(m), mMapMaker(mm), mCamera(c), mRelocaliser(mMap, mCamera),
      mirSize(irVideoSize) {
...
void Tracker::TrackFrame(Image<byte> &imFrame, bool bDraw) {
  ...
  if (mbDraw) {
    ...
    if (GV2.GetInt("Tracker.DrawFASTCorners", TRACKER_DRAW_FAST_CORNERS_DEFAULT,
                   SILENT)) {
      glColor3f(1, 0, 1);
      glPointSize(1);
      glBegin(GL_POINTS);
...
void Tracker::TakeKF(bool force) {
  if (mMap.IsGood()) {
    ...
    if (mTrackingQuality == GOOD &&
        (force ||
         (mMapMaker.NeedNewKeyFrame(mCurrentKF) &&
          ((double)(clock() - mnLastKeyFrameDroppedClock)) / CLOCKS_PER_SEC >
              minKFTimeDist &&
          mMapMaker.QueueSize() < 3))) {
      mMessageForUser << " Adding key-frame.";
      AddNewKeyFrame();
...
void Tracker::GUICommandHandler(string sCommand,
...
void Tracker::TrackForInitialMap() {
  ...
  if (mnInitialStage == TRAIL_TRACKING_STARTED) {
    ...
    if (mbUserPressedSpacebar) {
      ...
      bool succ = mMapMaker.InitFromStereo(
          mFirstKF, mCurrentKF, vMatches, mse3CamFromWorld,
          KFZeroDesiredCamFromWorld,
...
void Tracker::TrailTracking_Start() {
  ...
  mPreviousFrameKF =
      mFirstKF; // Always store the previous frame so married-matching can work.
}
...
void Tracker::TrackMap() {
  ...
  if (bTryCoarse &&
    ...
    } else { // ..otherwise choose nCoarseMax at random, again removing from the
             // PVS list.
  ...
  // so do all of these, with sub-pixel refinement.
  {
    ...
    for (unsigned int i = 0; i < avPVS[l].size(); i++)
      vIterationSet.push_back(avPVS[l][i]); // Again, plonk all searched points
                                            // onto the (maybe already populate)
                                            // vIterationSet.
  };
...
int Tracker::SearchForPoints(vector<TrackerData *> &vTD, int nRange,
  ...
  for (unsigned int i = 0; i < vTD.size(); i++) // for each point..
  {
    ...
    if (nSubPixIts > 0) {
      ...
      if (!bSubPixConverges) { // If subpix doesn't converge, the patch location
                               // is probably very dubious!
        TD.bFound = false;
        nFound--;
        manMeasFound[Finder.GetLevel()]--;
        continue;
...
Vector<6> Tracker::CalcPoseUpdate(vector<TrackerData *> vTD,
  ...
  for (unsigned int f = 0; f < vTD.size(); f++) {
    ...
    wls.add_mJ(v2[0], (Vector<6>)(TD.dSqrtInvNoise * m26Jac[0]),
               dWeight); // These two lines are currently
    wls.add_mJ(v2[1], (Vector<6>)(TD.dSqrtInvNoise * m26Jac[1]),
...
string Tracker::GetMessageForUser() { return mMessageForUser.str(); }
