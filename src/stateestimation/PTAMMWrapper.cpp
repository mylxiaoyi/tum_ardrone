...
pthread_mutex_t PTAMMWrapper::navInfoQueueCS = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t PTAMMWrapper::shallowMapCS = PTHREAD_MUTEX_INITIALIZER;

pthread_mutex_t PTAMMWrapper::logScalePairs_CS = PTHREAD_MUTEX_INITIALIZER;

PTAMMWrapper::PTAMMWrapper(DroneKalmanFilter* f, EstimationNode* nde)
{
	filter = f;
	node = nde;
...
void PTAMMWrapper::ResetInternal()
{
...
void PTAMMWrapper::setPTAMPars(double minKFTimeDist, double minKFWiggleDist, double minKFDist)
{
	if(mpMapMaker != 0)
...
PTAMMWrapper::~PTAMMWrapper(void)
{
	if(mpCamera != 0) delete mpCamera;
	if(mpMap != 0) delete mpMap;
	if(mpMapMaker != 0) delete mpMapMaker;
...
void PTAMMWrapper::renderGrid(TooN::SE3<> camFromWorld)
{
	myGLWindow->SetupViewport();
	myGLWindow->SetupVideoOrtho();
	myGLWindow->SetupVideoRasterPosAndZoom();
...
TooN::Vector<3> PTAMMWrapper::evalNavQue(unsigned int from, unsigned int to, bool* zCorrupted, bool* allCorrupted, float* out_start_pressure, float* out_end_pressure)
{
...
void PTAMMWrapper::newNavdata(ardrone_autonomy::Navdata* nav)
{
...
void PTAMMWrapper::newImage(sensor_msgs::ImageConstPtr img)
{
...
void PTAMMWrapper::on_key_down(int key)
{
	if(key == 114) // r
	{
...
bool PTAMMWrapper::handleCommand(std::string s)
{
...
void PTAMMWrapper::on_mouse_down(CVD::ImageRef where, int state, int button)
{
